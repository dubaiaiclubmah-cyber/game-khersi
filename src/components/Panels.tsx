        ...
        {DIFF_ORDER.map((key) => {
          ...
          return (
            <button
              ...
              className="clip-pixel-sm flex items-center justify-between gap-2 border bg-shell2 px-3 py-2.5 text-left transition-all duration-150 hover:-translate-y-0.5 hover:brightness-125 active:translate-y-0 cursor-pointer"
              style={{
    ...
    <section className="panel clip-pixel p-4">
      ...
      <div className="mt-3 flex items-end justify-between gap-2">
        ...
        <div className="text-right">
          ...
          <p className={"mt-1.5 font-display text-base leading-none " + (game.isNewBest ? "text-lime" : "text-teal")}>
            {fmt(game.best)}
          </p>
        </div>
      </div>
      ...
      {/* recent runs chart */}
      <div className="mt-4">
        <p className="font-display text-[7px] tracking-[0.2em] text-fern">{t.historyTitle}</p>
          ...
          return (
            ...
            >
              <span className="flex items-center gap-2">
                <span className="inline-block h-2 w-2" style={{ background: d.color, boxShadow: `0 0 8px ${d.color}88` }} />
...
function MedalIcon({ kind, dim }: { kind: MedalKind; dim?: boolean }) {
  ...
  switch (kind) {
    case "apple":
      return (
        <svg width="19" height="19" viewBox="0 0 24 24" style={style} aria-hidden>
          <circle cx="12" cy="14" r="8" fill="#ff6b5e" />
          <rect x="11" y="2" width="2" height="5" fill="#8a5a33" />
          <ellipse cx="16" cy="5" rx="3.4" ry="1.6" fill="#7be06a" transform="rotate(-24 16 5)" />
        </svg>
    ...
    case "clock":
      return (
        <svg width="19" height="19" viewBox="0 0 24 24" fill="none" style={style} aria-hidden>
  ...
  return (
    <div
      ...
      className={
        ...
        (unlocked
          ? "border-amber/70 bg-amber/20 shadow-[0_0_16px_rgba(255,201,77,0.18)] hover:-translate-y-0.5 hover:shadow-[0_0_22px_rgba(255,201,77,0.32)]"
...
export function MedalsPanel({ game }: { game: GameAPI }) {
  ...
  return (
    <section className="panel clip-pixel p-4">
      ...
      <p className="mt-3 text-[11px] leading-snug text-fern">{t.medalsNote}</p>
    </section>
  );
}
    ...
    <div className="panel clip-pixel flex w-full gap-1 p-1">
      {DIFF_ORDER.map((key) => {
        ...
        return (
          ...
          >
            <span className="block font-display text-[8px] tracking-[0.08em]" style={{ color: active ? d.color : "var(--color-mint)" }}>
              {t[DIFF_LABEL[key]]}
            </span>
            <span className="mt-1 block font-display text-[7px]" style={{ color: active ? d.color : "var(--color-fern)" }}>
              ×{d.mult}
            </span>
          </button>
        );
...
function Accordion({ title, children }: { title: string; children: ReactNode }) {
  ...
  return (
    <div className="panel clip-pixel w-full">
      ...
      >
        <span className="font-display text-[8px] tracking-[0.26em] text-mint">{title}</span>
        <svg
          width="12"
          height="12"
          viewBox="0 0 24 24"
      ...
      {open && <div className="border-t border-hedge px-4 py-3.5">{children}</div>}
    </div>
  );
}
            ...
            return (
              <li key={key} className="flex items-center justify-between">
                <span className="flex items-center gap-2">
                  ...
                  <span className={"font-display text-[8px] " + (active ? "text-fog" : "text-fern")}>
                    {t[DIFF_LABEL[key]]}
                  </span>
                </span>
                <span className="font-display text-[10px]" style={{ color: d.color }}>
